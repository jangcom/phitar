...
my %_hash_refs = ( # (key) attribute => (val) default
    curr_v_source_param => sub { {} },
    curr_source_dist    => sub { {} },
    constrained_args    => sub { {} },
...
#    $mapdl_tab->Data->set_plane_sep("\n");
    ...
    foreach my $ang (@ang_fnames) {
        ...
        foreach my $newpage (0..$newpage_idx) {
            ...
            # x: Do NOT transpose.
            for (my $i=0; $i<=$#{$mapdl_tab->Data->row_idx->[$newpage]}; $i++) {
                printf(
                    "%s%s",
...
#        msg   => qr/^\s*ms[ud][lcr]:\s*/i, # Replaced by 'noms' of ANGEL
    ...
    foreach my $ang (@ang_fnames) {
        next if -d;
        next if not $ang; # An empty string can be passed from the main program
        ...
        # (2) Take the labels of user-defined constants for cluster plot params.
        ...
        foreach my $line (<$ang_fh>) {
            ...
            if ($ang =~ /(?:g|3d)show/i and $line =~ /legs\[.*\]/) {
                ...
                my $_new_fac = $ang =~ /gshow/i ? 1 : 1.4; # Bigger fac for 3D
                $line =~ s/
                    (?<bef>legs\[)
                    .*
                    (?<aft>\])
            ...
            #************************************************************
            if (
                $line =~ /^\s*\w+:\s*Fluence\s*\(cm\^\{-2\}\s*source\^\{-1\}\)/i
            ) {
                push @{$modified{mc_fluence}{bef}}, $line;
...
sub modify_eps_files {
    ...
    foreach my $eps (@{$eps_fnames_aref}) {
        ...
        foreach my $line (<$eps_fh>) {
            ...
            if ($line =~ /^\s*\/str[0-9]+\s*\(\s*[+\-]?[0-9.]+E[+\-][0-9.]+/i) {
            ...
            if ($line =~ /str004\s*xps\s*($wrong_pos)/i) {
                ...
                if ($line ne $modified{num_partial_supersc}{bef}[-1]) {
                    push @{$modified{num_partial_supersc}{aft}},      $line;
                    push @{$modified{num_partial_supersc}{line_num}}, $line_num;
                    $modified{any}++;
                }
            }
...
# Source energy setters
# - cylindrical
# - gaussian_xyz (xyz FWHMs are set independently)
# - gaussian_xy (xy FWHMs are set simultaneously)
sub set_nrg_val_fixed {
    my $self = shift;
    
    $self->type_of_int->{nrg}{val_fixed} = $_[0] if defined $_[0];
    
    return;
}
sub set_nrg_vals_of_int {
    my $self = shift;
    
    @{$self->type_of_int->{nrg}{vals_of_int}} = @{$_[0]} if defined $_[0];
    
    return;
}

# Setters for cylindrical source
sub set_rad_val_fixed {
    my $self = shift;
    
    $self->type_of_int->{rad}{val_fixed} = $_[0] if defined $_[0];
...
sub set_rad_vals_of_int {
    ...
    @{$self->type_of_int->{rad}{vals_of_int}} = @{$_[0]} if defined $_[0];
    
    return;
}

# Setters for gaussian_xyz
# x-FWHM
sub set_x_fwhm_val_fixed {
    my $self = shift;
    
    $self->type_of_int->{x_fwhm}{val_fixed} = $_[0] if defined $_[0];
...
sub set_x_fwhm_vals_of_int {
    ...
    @{$self->type_of_int->{x_fwhm}{vals_of_int}} = @{$_[0]} if defined $_[0];
...
sub set_y_fwhm_val_fixed {
    ...
    $self->type_of_int->{y_fwhm}{val_fixed} = $_[0] if defined $_[0];
...
sub set_y_fwhm_vals_of_int {
    ...
    @{$self->type_of_int->{y_fwhm}{vals_of_int}} = @{$_[0]} if defined $_[0];
...
sub set_z_fwhm_val_fixed {
    ...
    $self->type_of_int->{z_fwhm}{val_fixed} = $_[0] if defined $_[0];
...
sub set_z_fwhm_vals_of_int {
    ...
    @{$self->type_of_int->{z_fwhm}{vals_of_int}} = @{$_[0]} if defined $_[0];
    
    return;
}

# Setters for gaussian_xy
# x-FWHM and y-FWHM
sub set_xy_fwhms_val_fixed {
    my $self = shift;
    
    $self->type_of_int->{xy_fwhms}{val_fixed} = $_[0] if defined $_[0];
...
sub set_xy_fwhms_vals_of_int {
    ...
    @{$self->type_of_int->{xy_fwhms}{vals_of_int}} = @{$_[0]} if defined $_[0];
