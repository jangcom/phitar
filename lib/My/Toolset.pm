...
sub calc_area {
    ...
    my %shapes = (
        # Circular
        circ => {
            # Keys are used only for argument validation
    ...
    foreach my $dim (sort keys %{$shapes{$shape}{dims}}) {
        if (not exists $dims{$dim}) {
            croak "\n[$dim] of [$shape] is missing! we need".
...
sub calc_vol {
    ...
    my %shapes = (
        # Rectangular cuboid
        # > aka rectangular parallelepiped
        # > Macrobody mnemonic: RPP
        rpp => {
            # Keys are used only for argument validation.
            dims => { map { $_ => undef } qw(xmin xmax ymin ymax zmin zmax) },
            # The equation below will be 'eval'ed; look up %dims for ref.
            eq   => 'abs($dims{xmax} - $dims{xmin})'.
        ...
        trc => {
            dims => { map { $_ => undef } qw(bot_radius top_radius height) },
            eq   => 'PI/3 * ('.
                        '$dims{bot_radius}**2'.
                        '+ $dims{bot_radius} * $dims{top_radius}'.
                        '+ $dims{top_radius}**2'.
                    ') * $dims{height}',
        },
    );
